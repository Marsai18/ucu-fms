...
export const markAsRead = async (req, res, next) => {
  try {
    ...
    const canAccess =
      ...
      (req.user?.role === 'hod' && notification.recipientRole === 'hod') ||
      ((req.user?.role === 'admin' || req.user?.username === 'masai') && notification.recipientRole === 'admin');
...
export const markAllAsRead = async (req, res, next) => {
  try {
    if (req.user?.role === 'driver' && req.user?.driverId) {
      ...
      await db.markNotificationsRead(null, 'hod', null);
    } else if (req.user?.role === 'admin' || req.user?.username === 'masai') {
      await db.markNotificationsRead(null, 'admin', null);
    }
    res.json({ ok: true });
  } catch (error) {
    next(error);
